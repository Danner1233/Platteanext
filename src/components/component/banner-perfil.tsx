...
export function BannerPerfil() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await fetch(`${SERVER_URL}/api/persona/${userId}`, {
          headers: {
  ...
  console.log(`${process.env.SERVER_URL}${profile?.bannerPersona}`)
  if (error) return <p>Error: {error}</p>;
  return (
    <Card className="w-full rounded-2x2 overflow-hidden">
      <div className="h-70 bg-[#1e1e1e] relative flex justify-center">
        <img
          src={`${process.env.SERVER_URL}/${profile?.bannerPersona}`} 
          // Concatenar la URL base con la URL específica de la imagen
          alt="Banner"
      ...
      <div className="p-6 pt-12 bg-white text-black">
        <div className="flex items-center gap-4">
          <Avatar className="w-24 h-24 border-4 border-white">
            <AvatarImage src={`${process.env.SERVER_URL}/${profile?.FotoPersona}` || "/OIP.jpeg"} alt="User Avatar" />
          </Avatar>
